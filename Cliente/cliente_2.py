...
clientSocketUDP.bind(('localhost', PORT+1))
...
def enviar_pacote():
    ...
    if seq:
        msg = b'\x01' + conteudo + calcular_checksum(conteudo)
    else:
        msg = b'\x00' + conteudo + calcular_checksum(conteudo)
...
def receber_dados():
    ...
    seq_bin = b'\x01' if seq else b'\x00'
...
thread_escuta = threading.Thread(target=escutar_porta, args=(PORT+2,))
thread_escuta.start()
